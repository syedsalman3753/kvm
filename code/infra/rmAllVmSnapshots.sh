...
userCh(){
  ...
  case $choice in
    ...
    ;;
    *)
      echo "$(tput setaf 1) VM SnapShorts Remove Operation not performed; EXITING $(tput sgr0) ";
      exit 1;
    ;;
  esac
}
...
echo -e "\n$(tput setaf 2)USAGE: bash rmAllVmSnapshots.sh vmlistFilename chkpt_name\n$(tput sgr0)";
echo "$(tput setaf 4)This script will remove particular SnapShorts for the vm's present in input file $(tput sgr0)";
...
for vm in $vmList; do
  echo -e "\n $(tput setaf 6)[ $vm ] $(tput sgr0)";
	...
	echo -n "   $(tput setaf 3)"
	virsh snapshot-delete $vm "$vm-$chkpt.kvmSnpSht"
	echo "$(tput sgr0)";
done	

echo "$(tput setaf 2) Snapshots of All vm's successfully removed !!! $(tput sgr0)"